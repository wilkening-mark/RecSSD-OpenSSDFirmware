...
unsigned int debugRequestsEnable;
...
void InitDieReqQueue()
{
	...
	transReqQueue = (struct transReqArray*)(TRANS_REQ_QUEUE_ADDR);
	transReadReqQueue = (struct transReadReqArray*)(TRANS_READ_REQ_QUEUE_ADDR);
	transRqPointer = (struct transRqPointerEntry*)(TRANS_REQ_QUEUE_POINTER_ADDR);
	transReadRqPointer = (struct transRqPointerEntry*)(TRANS_READ_REQ_QUEUE_POINTER_ADDR);
	transPageReqQueue = (struct transPageReqArray*)(TRANS_PAGE_REQ_QUEUE_ADDR);
	...
	transRqPointer->head = 0xffff;
	transRqPointer->tail = 0xffff;
	transRqPointer->current = 0xffff;
	transRqPointer->availhead = 0;
	transRqPointer->availtail = TRANS_REQ_QUEUE_DEPTH-1;
	for(queueDepth=0; queueDepth<TRANS_REQ_QUEUE_DEPTH; queueDepth++)
	{
		transReqQueue->transReqEntry[queueDepth].entryIdx = 0;

		transReqQueue->transReqEntry[queueDepth].prev = (queueDepth==0) ? 0xffff : queueDepth-1;
		transReqQueue->transReqEntry[queueDepth].next = (queueDepth==TRANS_REQ_QUEUE_DEPTH-1) ? 0xffff : queueDepth+1;
	}

	transReadRqPointer->head = 0xffff;
	transReadRqPointer->tail = 0xffff;
	transReadRqPointer->current = 0xffff;
	transReadRqPointer->availhead = 0;
	transReadRqPointer->availtail = TRANS_READ_REQ_QUEUE_DEPTH-1;
	for(queueDepth=0; queueDepth<TRANS_READ_REQ_QUEUE_DEPTH; queueDepth++)
	{
		transReadReqQueue->transReqEntry[queueDepth].entryIdx = 0;
		transReadReqQueue->transReqEntry[queueDepth].nextPage = 0;

		transReadReqQueue->transReqEntry[queueDepth].prev = (queueDepth==0) ? 0xffff : queueDepth-1;
		transReadReqQueue->transReqEntry[queueDepth].next = (queueDepth==TRANS_READ_REQ_QUEUE_DEPTH-1) ? 0xffff : queueDepth+1;
	}
}
