...
#define TRANS_CONFIG_ADDR   0x12500000
#define TRANS_BUF_ADDR      (TRANS_CONFIG_ADDR + TRANS_BUF_ENTRY_NUM * TRANS_CONFIG_SIZE)
// End Trans Buf 0x13500000

#define TRANS_EMBED_CACHE_ADDR (TRANS_BUF_ADDR + TRANS_BUF_ENTRY_NUM * TRANS_BUF_ENTRY_SIZE)

// for buffers
#define BUFFER_MAP_ADDR		 (TRANS_EMBED_CACHE_ADDR + sizeof(struct transEmbedCacheEntry) * TRANS_EMBED_CACHE_ENTRY_NUM)  // OLD -> 0x1A600000
#define BUFFER_LRU_LIST_ADDR 	(BUFFER_MAP_ADDR + sizeof(struct bufEntry) * BUF_ENTRY_NUM)
#define TRANS_BUF_MAP_ADDR (BUFFER_LRU_LIST_ADDR + sizeof(struct bufLruEntry) * DIE_NUM)
#define TRANS_AVAIL_Q_ADDR (TRANS_BUF_MAP_ADDR + sizeof(struct transBufEntry) * TRANS_BUF_ENTRY_NUM)

// for map tables
#define PAGE_MAP_ADDR	(TRANS_AVAIL_Q_ADDR + sizeof(struct transBufAvailQueue))
...
// for trans request queues
#define TRANS_REQ_QUEUE_ADDR (SUB_REQ_QUEUE_POINTER_ADDR+ sizeof(struct rqPointerEntry) * DIE_NUM)
#define TRANS_READ_REQ_QUEUE_ADDR (TRANS_REQ_QUEUE_ADDR+ sizeof(struct transReqEntry) * TRANS_REQ_QUEUE_DEPTH)
#define TRANS_REQ_QUEUE_POINTER_ADDR (TRANS_READ_REQ_QUEUE_ADDR+ sizeof(struct transReqEntry) * TRANS_READ_REQ_QUEUE_DEPTH)
#define TRANS_READ_REQ_QUEUE_POINTER_ADDR (TRANS_REQ_QUEUE_POINTER_ADDR + sizeof(struct transRqPointerEntry))
#define TRANS_PAGE_REQ_QUEUE_ADDR (TRANS_READ_REQ_QUEUE_POINTER_ADDR+ sizeof(struct transRqPointerEntry))

// for low level scheduler
#define DIE_STATUS_TABLE_ADDR	(TRANS_PAGE_REQ_QUEUE_ADDR+ sizeof(struct transPageReqEntry) * DIE_NUM)
...
#define TRANS_STATS_ADDR (WAY_PRIORITY_TABLE_ADDR + sizeof(struct wayPriorityArray))
